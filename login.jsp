...
try {
    ...
    if (rs.next()) {
        ...
        response.sendRedirect("index3.html?ctnm="+sid);
    //request.getRequestDispatcher("index1.html").forward(request, response);
        } else {
            // Passwords do not match
            response.sendRedirect("login.html?error=Invalid_password"); // Redirect to login page with error message
        }
    } else {
        // No user found with the given email
        response.sendRedirect("login.html?error=User_not_found"); // Redirect to login page with error message
    }
